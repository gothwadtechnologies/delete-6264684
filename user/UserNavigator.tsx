...
import ProgressScreen from '../common/screens/tabs/ProgressScreen.tsx';
...
const UserNavigator: React.FC<UserNavigatorProps> = ({ 
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'Home': return <ProgressScreen user={user} />;
      default: return null;
    }
  };
      ...
      <header className="bg-[#0288D1] px-4 pt-3 pb-2 shadow-lg shadow-blue-900/20 shrink-0 z-[60] flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[8px] font-bold text-blue-200 uppercase tracking-[0.2em] mt-0.5">
                {forcedResource ? 'Resource Center' : 'Student Portal'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onOpenNotifications} className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center text-white active:scale-90 transition-all relative border border-white/10">
              <Bell className="w-4 h-4" />
              <span className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-rose-500 rounded-full border border-blue-800"></span>
            </button>
            <button onClick={onOpenProfile} className="w-9 h-9 rounded-full bg-white shadow-md active:scale-90 transition-all flex items-center justify-center border border-blue-200 overflow-hidden">
               <img 
                 src={APP_LOGO} 
                 alt="Profile" 
