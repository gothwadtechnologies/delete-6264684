...
import { BRANDING_FOOTER, APP_LOGO } from '../common/constants';
...
const UserDrawer: React.FC<UserDrawerProps> = ({ 
  ...
  return (
    <>
      ...
      <div 
        className={`absolute top-0 bottom-0 left-0 w-[75%] max-w-[280px] bg-[#0288D1] z-[110] transform shadow-2xl border-r border-blue-900 flex flex-col ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <button 
          onClick={() => handleAction('profile')}
          className="p-6 bg-[#0288D1] border-b border-blue-900 shrink-0 flex items-center gap-4 text-left active:bg-white/5 transition-colors"
        >
          <div className="w-12 h-12 bg-white/20 text-white border-white/30 rounded-full flex items-center justify-center text-xl font-black shadow-sm border shrink-0 overflow-hidden">
            <img 
              src={APP_LOGO} 
              alt="Profile" 
        ...
        <div className="p-4 border-t border-blue-900 bg-[#01579B] shrink-0">
          {!showLogoutConfirm ? (
            <button 
